...
<script type="text/javascript">
	$(document).ready(function(){
		...
		if(msg!=""){
			if(msg=="수정되었습니다"){
				if(confirm(msg+"\n조회화면이동하시겠습니까?")){
					location.href="${path}/output.do?method=list";
				}	
			}
			if(msg=="삭제되었습니다."){
		...
		$("#delBtn").click(function(){
			if(sessId!=$("[name=writer]").val()){
				...
				if(confirm("삭제하시겠습니까?")){
					location.href="${path}/output.do?method=del&output_list_num="+$("[name=output_list_num]").val();	
				}
			}
		});
		$("#uptBtn").click(function(){
			if(sessId!=$("[name=writer]").val()){
				...
				if(confirm("수정하시겠습니까?")){
					$("form").attr("action","${path}/output.do?method=upt");
					$("form").submit();
				}
			}
		});
		$("#reBtn").click(function(){
			if(confirm("답글을 작성하시겠습니까?")){
				...
				$("[name=output_refno]").val($("[name=output_list_num]").val());
				$("[name=output_title]").val("RE:"+$("[name=output_title]").val());
				$("[name=output_content]").val("\n\n\n\n\n\n\======이전글======\n"+$("[name=output_content]").val());
				$("form").attr("action","${path}/output.do?method=insertFrm");
				$("form").submit();
			}
		});
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">산출물관리 상세</h1>
                    </div>
            ...
            <!-- Content Row -->
				...
				<div class="container">
					<form method="post" enctype="multipart/form-data"
						action="${path}/output.do?method=upt">
						
					<div class="input-group mb-3 fileCls">	
						<div class="input-group-prepend">
							<span class="input-group-text">프로젝트명</span>
						</div>
							<input name="project_name" class="form-control" value="${output.project_name}"
									placeholder="작성자입력하세요" readonly /> 
						<div class="input-group-prepend">
							<span class="input-group-text">부서명</span>
						</div>
							<input name="dname" class="form-control" value="${output.dname}" readonly/>	
					</div>											
					<div class="input-group mb-3 fileCls">	
							<div class="input-group-prepend">
							...
							<input name="writer" class="form-control" 
								placeholder="작성자입력하세요"   value="${output.writer}" readonly/>
							<!-- 작성자는 입력해서 변경불가능 처리 -->	
							<div class="input-group-prepend">
								<span class="input-group-text">조회수</span>
							</div>
							<input name="readcnt" class="form-control" value="${output.readcnt}" readonly/>	
					</div>			
					<div class="input-group mb-3 fileCls">	
						<div class="input-group-prepend">
							<span class="input-group-text">글번호</span>
						</div>
						<input name="output_list_num" class="form-control" value="${output.output_list_num}" readonly/>	
						<div class="input-group-prepend">
							<span class="input-group-text">상위글번호</span>
						</div>
						<input name="output_refno" class="form-control" value="${output.output_refno}" readonly/>	
					</div>			
					...
					<div class="input-group mb-3">
						...
						<input name="output_title" class="form-control" value="${output.output_title}" />			 
					</div>  
					...
					<div class="input-group mb-3 fileCls">	
						...
						<input class="form-control" 
							placeholder="작성자입력하세요"  readonly
							value='<fmt:formatDate type="both" value="${output.output_reg}"  />'/>	
						<div class="input-group-prepend">
							<span class="input-group-text">수 정일</span>
						</div>
						<input  class="form-control" readonly
							value='<fmt:formatDate type="both" value="${output.output_upt}" />'/>	
					</div>			
					<div class="input-group mb-3 fileCls">
						<div class="input-group-prepend">
							<span class="input-group-text">내 용</span>
						</div>
						<textarea name="output_content" rows="10" 
							class="form-control" 
							placeholder="내용입력하세요" >${output.output_content}</textarea>		 
					</div> 
					<c:forEach var="fname" items="${output.fnames}">
					<div class="input-group mb-3 fileCls">
						<div class="input-group-prepend">
