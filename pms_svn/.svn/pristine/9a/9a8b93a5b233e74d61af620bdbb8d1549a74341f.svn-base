...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#regBtn").click(function(){
			// 등록 처리 유효성체크
			if(confirm("등록하시겠습니까?")){
				if($("[name=notice_title]").val()==""){
					alert("제목은 필수항목입니다.")
					$("[name=notice_title]").focus();
					return;
				}
				console.log(notice.level);
				$("form").submit();
			}
		});
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            -->
							...
							<div class="container">
								<form method="post" enctype="multipart/form-data" action="${path}/notice.do?method=insert">
							<div class="input-group mb-3 fileCls">	
								<div class="input-group-prepend">
									<span class="input-group-text">프로젝트명</span>
								</div>
								
									<c:if test="${notice.notice_refno >1}">
										<input name="project_name" class="form-control" value="${notice.project_name}" readonly/>
									</c:if>
									<c:if test="${notice.notice_refno == 0}">
							...
							<div class="input-group mb-3">
								...
								<input name="notice_title" class="form-control" value="${notice.notice_title}" placeholder="제목입력하세요"/>			 
							</div>  	
							<div class="input-group mb-3 fileCls">
								<div class="input-group-prepend">
